...
async def process_image(
        request: Request,
        instrument_name: str = Form(None),
        temp: int = Form(None),
        music_sheet: UploadFile = File(None),
        notes_data: str = Form(None)
):
    try:
        if music_sheet is not None:
        ...
        if notes_data is not None:
            ...
            music_file_path = os.path.join(MUSIC_DIR, "music.wav")
            # fs = FluidSynth()
            # fs.midi_to_audio(result_file_path, music_file_path)
